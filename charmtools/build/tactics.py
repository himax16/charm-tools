...
class WheelhouseTactic(ExactMatch, Tactic):
    ...
    def _process_per_layer(self, wheelhouse):
        # HIMA: git format-patch -1 COMMIT and git am < some.patch for the vergit
        # https://github.com/juju/charm-tools/COMMIT.patch
    ...
    def _process_combined(self, wheelhouse):
        ...
        with utils.tempdir(chdir=False) as temp_dir:
            ...
            self._add(
                wheelhouse, '-r', wh_file, wh_cons_file,
    ...
    def sign(self):
        ...
        if self.per_layer:
            ...
            sigs[self.CONS_FILENAME] = (
                ...
                if (self.target.directory / self.CONS_FILENAME).exists()
                else None,
            )
